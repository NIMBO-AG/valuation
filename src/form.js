...
function FormComponent() {
  ...
  if (isSubmitted) {
    return e('div',{className:'text-center'},
      e(LanguageSwitcher,{
        currentLang: lang,
        onChange: nl => { params.set('lang', nl); window.history.replaceState(null,'','?'+params.toString()); setLang(nl); }
      }),
      e('p',{}, translations.thankYou),
      e('a',{href:window.location.href.replace('&submitted=true',''), className:'text-blue-600 underline'},
  ...
  const visibleBlocks = blocks.filter(b => {
    ...
    if (updateMode) {
      ...
      if (um === 'only in update mode') return false;
    }
    // Free-Mode Regeln
    ...
    if (freeMode) {
      ...
      if (fm === 'only in free mode') return false;
    }
    // visible_if wie gehabt
    if (!b.visible_if) return true;
    try {
      const cond = b.visible_if.trim();
      const m = cond.match(/^(\w+)\s*==\s*"(.+)"$/);
      if (m) {
        return answers[m[1]] === m[2];
      }
    } catch(err) {
      console.warn('Invalid visible_if:', b.visible_if);
    }
    return true;
  });
  ...
  return e('div',{},
    ...
    e('form',{onSubmit:handleSubmit,className:'space-y-4'},
      visibleBlocks.map(b=>
        e(React.Fragment,{key:b.id},
          renderQuestion(
            b,
            answers[b.id]|| (b.type==='checkbox'?[]:''),
            v=> setAnswers({...answers,[b.id]:v}),
            translations,
            lang
          )
        )
