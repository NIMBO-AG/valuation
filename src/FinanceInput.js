  ...
  function FinanceInput({ answers, setAnswers }) {
    ...
    const explanations = {
      ...
      'Einzelgeschäftsführung':
        'Ist Ihr Unternehmen von genau einer Person geführt? ' +
        'Wählen Sie „Nein“ bei mehreren Geschäftsführer:innen/Partner:innen.',
      'CEO-Saläre':              'Summe aller GF-Löhne pro Jahr.',
      'Abschreibungen':          'Wertminderungen auf Sach-/IMM-Vermögen.',
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // 7) Render a single row + its explanation
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // 8) Build wizard steps
    // ─────────────────────────────────────────────────────────────────────────
    const rows = [];

    // Step 1: Umsatz
    rows.push(
      ...renderDataRow({ label: 'Umsatz', key: 'Umsatz', isInput: true })
    );

    // Step 2: CEO-Saläre (only if question answered “Ja”)
    if (revComplete && singleAnswer === 'Ja') {
      rows.push(
        ...renderDataRow({ label: 'CEO-Saläre', key: 'CEO-Saläre', isInput: true })
      );
    }

    // **Always** after the CEO-Saläre row, show the question row (once Umsatz is complete)**
    if (revComplete) {
      rows.push(
    ...
    // Step 3: Abschreibungen
    if (revComplete && singleAnswer && salaryComplete) {
      rows.push(
    ...
    if (revComplete && singleAnswer && salaryComplete && depComplete && ebitComplete) {
      rows.push(
        ...renderDataRow({
          ...
          key:    'EBIT-Marge',
          isInput: false,
          calcFn: calcEBITMargin
        })
      );
    }

    // Step 6: EBIT Anpassung
    if (revComplete && singleAnswer && salaryComplete && depComplete && ebitComplete) {
      rows.push(
    ...
    // Step 7: Angepasstes EBIT + EBITC
    if (revComplete && singleAnswer && salaryComplete && depComplete && ebitComplete && adjComplete) {
      rows.push(
        ...renderDataRow({
          label:  'EBIT angepasst',
          key:    'EBIT angepasst',
          isInput: false,
          calcFn: calcAdjustedEBIT
        }),
        ...renderDataRow({
          label:  'EBITC (EBIT+CEO)',
          key:    'EBITC',
          isInput: false,
          calcFn: calcEBITC
        })
    ...
    return e('div', { className: 'space-y-6' },
      ...
      e('table', { className: 'w-full table-auto border-collapse text-sm' },
        e('thead', {},
          e('tr', {},
            e('th', {
              className: 'px-2 py-1 bg-gray-200 text-left'
            }, 'Posten'),
            ...years.map(y =>
              e('th', {
                key: y,
                className: 'px-2 py-1 bg-gray-200 text-center'
              }, y)
            )
          )
        ),
